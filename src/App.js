...
function App() {
  ...
  if (screen === 'main') return (
    <div className="container" style={{background: theme.bg, minHeight: '100vh', padding: '20px 15px', transition: '0.3s'}}>
            ...
            return [...regularGames, ...archiveGames].slice(0, 10).map((g) => (
              <div key={g.id} style={{display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '12px 15px', borderBottom: `1px solid ${theme.border}`}}>
                ...
                <div style={{display: 'flex', gap: '10px', alignItems: 'center'}}>
                  ...
                  <div onClick={() => { if(prompt("Пароль:")==="2910") remove(ref(db, `games_history/${g.id}`)) }} style={{cursor: 'pointer', fontSize: '16px', opacity: 0.3}}>🗑️</div>
                </div>
              </div>
            ));
          })()}
      ...
      {gameStatus === 'active' ? (
        <button className="start-btn" onClick={() => setScreen('game')} style={{marginTop: '25px', width: '100%', padding: '16px', borderRadius: '16px', background: '#fdcb6e', color: '#2d3436', fontSize: '16px', fontWeight: 'bold', border: 'none'}}>👁️ Приєднатися як глядач</button>
      ) : (
        <button className="start-btn" onClick={() => setScreen('select-role')} style={{marginTop: '25px', width: '100%', padding: '16px', borderRadius: '16px', background: darkMode ? '#6c5ce7' : '#2d3436', color: 'white', fontSize: '16px', fontWeight: 'bold', border: 'none'}}>Нова гра</button>
      )}
    </div>
  );
  ...
  if (screen === 'game') {
    ...
    return (
      <div className="container" style={{maxWidth: '100%', padding: '10px', background: theme.bg, minHeight: '100vh'}}>
        {winners.length > 0 && (
    ...
    <div className="container" style={{background: theme.bg, minHeight: '100vh', padding: '20px'}}>
      ...
      <div style={{display: 'flex', flexDirection: 'column', gap: '10px', marginBottom: '20px'}}>
        {Object.values(lobbyPlayers).map(p => (
          <div key={p.name} style={{display: 'flex', justifyContent: 'space-between', alignItems: 'center', background: theme.card, color: theme.text, padding: '15px', borderRadius: '12px'}}>
            <span>✅ {p.name}</span>
